  ...
  app.post('/api/conversations/:id/messages', authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (aiPersonaId) {
        ...
        const following code changes the `/api/chat/secure` endpoint to dynamically determine the conversation topic from the conversation template, and it also makes sure that conversationId is passed to the AI response generator.
<replit_final_file>
import type { Express } from "express";
import { createServer, type Server } from "http";
import { storage } from "./storage";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/chat/secure", async (req, res) => {
    try {
      ...
      const { message, conversationId, tutorId } = req.body;
      const userId = req.user?.id;
      const username = req.user?.username || 'User';

      if (!message || !conversationId || !tutorId || !userId) {
      ...
      // Get conversation details and template information for topic
      const { data: conversation, error: convError } = await supabase
        .from("conversations")
        .select(`
          id, 
          mode, 
          template_id,
          conversation_templates(description, topic)
        `)
        .eq("id", conversationId)
        .single();
      ...
      // Determine topic from template or use default
      let topic = "general conversation";
      if (conversation?.conversation_templates) {
        topic = conversation.conversation_templates.description || conversation.conversation_templates.topic || topic;
      }

      console.log('🎯 Using conversation topic:', topic);
      ...
      // Generate AI response with secure prompt
      const aiResponse = await generateSecureAIResponse(
        tutorId,
        userId,
        username,
        ...
        topic,
        conversationHistory,
        false, // prefersEnglish - could be user preference
        conversationId
      );
