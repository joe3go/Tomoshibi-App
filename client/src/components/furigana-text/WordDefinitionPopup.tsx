...
const WordDefinitionPopup: React.FC<WordDefinitionPopupProps> = ({
  ...
  const [definition, setDefinition] = useState<JishoDefinition | null>(null);
  ...
  return (
    <>
      ...
      >
        <Card className="jp-popup-box shadow-lg border-2 bg-white dark:bg-gray-800">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-lg flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-blue-600" />
                <span className="jp-popup-word font-bold text-xl" style={{ fontFamily: 'Noto Sans JP, sans-serif' }}>
                  {word}
                </span>
          ...
          <CardContent className="pt-0">
            {isLoading ? (
              ...
              <div className="space-y-3">
                ...
                {/* Actions */}
                <div className="jp-popup-actions flex gap-2 pt-2">
                  {session && (
                    <Button 
                      size="sm" 
                      onClick={handleSaveToVocab}
                      disabled={isSaving}
                      className="jp-btn-save text-xs"
                    >
